...
export default function Home() {
  ...
  // For streamable modes (summarize/generate): reads the response as plain
  // text chunks and calls onChunk as each piece arrives, so the UI can show
  // text appearing progressively instead of waiting for the whole thing.
  async function streamApi(body, onChunk) {
    ...
    // If the server responded with JSON, it's an error (or a non-streaming
    // fallback) - handle it the same way callApi does.
    ...
    try {
      if (mode === 'summarize') {
        ...
        if (sumBriefMode) {
          ...
          setOutput({ title: 'Summary', kind: 'text', text: '' });
          const result = await streamApi(
            ...
            (partial) => setOutput({ title: 'Summary', kind: 'text', text: partial })
          );
        ...
        setOutput({ title: genType, kind: 'text', text: '' });
        const result = await streamApi(
          { mode: 'generate', brief: genBrief, type: genType, tone: genTone },
          (partial) => setOutput({ title: genType, kind: 'text', text: partial })
        );
